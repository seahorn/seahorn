...
static inline bool correctTypeAllOf(Expr exp, TypeChecker &tc) {
  return correctTypeAny<T, types...>(exp->first(), tc) && sameType(exp, tc);
}


// ensures: 1. correct number of children, 2. child1 is of type T, child 2 is
// the second type etc.
...
checkChildrenSpecific(Expr exp, TypeChecker &tc,
  ...
  if (checkNumChildren<compareType, numChildren>(exp) &&
      correctTypeSpecific<T, types...>(exp, tc, 0))
...
checkChildren(Expr exp, TypeChecker &tc,
  ...
  if (checkNumChildren<compareType, numChildren>(exp) &&
      correctTypeAllOf<T, types...>(exp, tc))
    return returnType(exp, tc);
  else
    return sort::errorTy(exp->efac());
}
...
template <typename T>
static inline std::function<Expr(Expr, TypeChecker &)> getReturnFn() {
  return [](Expr exp, TypeChecker &tc) { return mk<T>(exp->efac()); };
}
