...
static llvm::cl::opt<bool> DsaInfoToOutputVerbose(
    "dsa-info-verbose",
    llvm::cl::desc("Llvm DSA: print verbose information about DSA"),
    llvm::cl::init(false), llvm::cl::Hidden);

static llvm::cl::opt<std::string> DsaInfoToFile(
    "dsa-info-to-file",
...
void DsaInfoPass::addNode(const DSNode *n) {
...
void DsaInfoPass::insertReferrersMap(const DSNode *n, const Value *v) {
...
bool DsaInfoPass::hasReferrers(const DSNode *n) const {
  return m_referrers_map.find(n) != m_referrers_map.end();
}

const DsaInfoPass::ValueSet &DsaInfoPass::getReferrers(const DSNode *n) const {
  auto it = m_referrers_map.find(n);
  assert(hasReferrers(n));
  return it->second;
}

bool DsaInfoPass::addAllocSite(const Value *v, unsigned int &site_id) {
...
void DsaInfoPass::countAccesses(const DataLayout *dl,
                                const TargetLibraryInfo *tli, DSGraph *dsg,
                                Value *V) {

  const DSNode *n = dsg->getNodeForValue(V).getNode();
  if (!n)
    n = m_gDsg->getNodeForValue(V).getNode();
  if (!n)
    return;
...
DsaInfoPass::DsaInfoPass()
    : llvm::ModulePass(ID), m_dsa(nullptr), m_gDsg(nullptr) {}

unsigned int DsaInfoPass::getDSNodeID(const DSNode *n) const {
  auto it = m_nodes.find(n);
  assert(it != m_nodes.end());
  return it->second.m_id;
}

bool DsaInfoPass::isAccessed(const DSNode *n) const {
  auto it = m_nodes.find(n);
  assert(it != m_nodes.end());
  return it->second.m_accesses > 0;
}

const unsigned int DsaInfoPass::getAllocSiteID(const Value *V) const {
  auto it = m_alloc_sites.left.find(V);
  if (it != m_alloc_sites.left.end())
    return it->second;
  else
...
const Value *DsaInfoPass::getAllocValue(unsigned int alloc_site_id) const {
...
void DsaInfoPass::releaseMemory() {
  m_nodes.clear();
...
void DsaInfoPass::writeDSAnalysisInfo(llvm::raw_ostream &o) {
  ...
  if (DsaInfoToOutputVerbose) {
    ...
    std::sort(
        nodes_vector.begin(), nodes_vector.end(),
        [](WrapperDSNode n1, WrapperDSNode n2) { return (n1.m_id < n2.m_id); });

    for (auto &n : nodes_vector) {
      if (!hasReferrers(n.m_n))
        continue;
      ...
      if (nn->isNodeCompletelyFolded()) {
        ...
        if (++jj == ie) {
          ...
          if (ty_set_ptr->size() == 1) {
            ...
            svset<Type *>::const_iterator ki = (*ty_set_ptr).begin(),
                                          ke = (*ty_set_ptr).end();
            o << "[";
            for (; ki != ke;) {
              o << **ki;
      ...
      LOG(
          "dsa-info", o << "  "
                        << std::distance(referrers.begin(), referrers.end())
                        << " Referrers={\n";
          for (auto const &r
               : referrers) {
            if (r->hasName())
...
void DsaInfoPass::findDSNodeForValue(const Value *v,
...
void DsaInfoPass::writeAllocSitesInfo(llvm::raw_ostream &o, bool isFile) {
  ...
  for (auto p : m_alloc_sites.right) {
    ...
    if (nodes.size() > 1) {
      if (!isFile && DsaInfoToOutput) {
        o << "DsaInfoPass found an allocation site " << p.first
  ...
  for (auto &kv : m_nodes) {
    ...
    if (NodeID > 0) {
      if (isFile) {
        ...
        if (DsaInfoToOutputVerbose)
          errs() << "DsaInfoPass cannot find allocation site for DSNode ID="
                 << NodeID << "\n";
        num_nodes_without_alloc_site++;
        num_accesses_without_alloc_site += kv.second.m_accesses;
        LOG("dsa-info", const ValueSet &referrers = getReferrers(kv.second.m_n);
            for (auto const &r
                 : referrers) {
              if (r->hasName())
                o << "\t  " << r->getName();
...
bool DsaInfoPass::runOnModule(llvm::Module &M) {
  ...
  for (auto &F : M) {
    ...
    for (auto const &kv : boost::make_iterator_range(SM.begin(), SM.end())) {
  ...
  for (auto &kv : m_nodes) {
    ...
    if (!hasReferrers(n.m_n) || n.m_accesses == 0)
      continue;
  ...
  for (auto &GV : M.globals()) {
    ...
    if (!Ty->isSized()) {
      if (DsaInfoToOutput)
        errs() << "DsaInfoPass ignored unsized " << GV
               << " as allocation site\n";
      continue;
    }
    if (!GV.hasInitializer()) {
      if (DsaInfoToOutput)
        errs() << "DsaInfoPass ignored uninitialized " << GV
               << " as allocation site\n";
      continue;
    }
    if (GV.hasSection()) {
    ...
    for (inst_iterator i = inst_begin(F), e = inst_end(F); i != e; ++i) {
      ...
      if (AllocaInst *AI = dyn_cast<AllocaInst>(I)) {
        ...
        if (!Ty->isSized() || dl->getTypeAllocSize(Ty) <= 0) {
          if (DsaInfoToOutput)
            errs() << "DsaInfoPass ignored unsized " << *AI
                   << " as allocation site\n";
          continue;
        }
        unsigned int alloc_site_id;
...
void DsaInfoPass::getAnalysisUsage(llvm::AnalysisUsage &AU) const {
  AU.setPreservesAll();
  AU.addRequiredTransitive<llvm::SteensgaardDataStructures>();
  AU.addRequired<llvm::TargetLibraryInfoWrapperPass>();
}

} // end namespace llvm_dsa
} // end namespace seahorn
#endif

namespace seahorn {
namespace llvm_dsa {
char DsaInfoPass::ID = 0;

llvm::Pass *createDsaInfoPass() { return new DsaInfoPass(); }

static llvm::RegisterPass<DsaInfoPass> X("dsa-info",
                                         "Show information about DSA Nodes");
