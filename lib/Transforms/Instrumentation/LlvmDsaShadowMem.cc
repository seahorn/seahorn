...
template <typename Set> void markReachableNodes(const DSNode *n, Set &set) {
  if (!n)
    return;
...
template <typename Set> void retReachableNodes(const DSCallSite &cs, Set &set) {
...
void argReachableNodes(DSCallSite CS, DSGraph &dsg, Set1 &reach,
                       Set2 &outReach) {
...
static Value *getUniqueScalar(LLVMContext &ctx, IRBuilder<> &B,
                              const DSNode *n) {
...
bool LlvmDsaShadowMem::runOnFunction(Function &F) {
  ...
  for (BasicBlock &bb : F)
    for (Instruction &inst : bb) {
      if (const LoadInst *load = dyn_cast<LoadInst>(&inst)) {
        ...
        if (!n)
          n = gDsg->getNodeForValue(load->getOperand(0)).getNode();
        if (!n)
          continue;

        B.SetInsertPoint(&inst);
        B.CreateCall(m_memLoadFn,
                     {B.getInt32(getId(n)), B.CreateLoad(allocaForNode(n)),
                      getUniqueScalar(ctx, B, n)});
      } else if (const StoreInst *store = dyn_cast<StoreInst>(&inst)) {
        DSNode *n = dsg->getNodeForValue(store->getOperand(1)).getNode();
        if (!n)
          n = gDsg->getNodeForValue(store->getOperand(1)).getNode();
        if (!n)
          continue;
        B.SetInsertPoint(&inst);
        AllocaInst *v = allocaForNode(n);
        B.CreateStore(
            B.CreateCall(m_memStoreFn, {B.getInt32(getId(n)), B.CreateLoad(v),
                                        getUniqueScalar(ctx, B, n)}),
            v);
        ...
        for (const DSNode *n : reach) {
          ...
          // -- read/write or new node
            ...
            B.CreateStore(B.CreateCall(argFn, {B.getInt32(id), B.CreateLoad(v),
                                               B.getInt32(idx),
                                               getUniqueScalar(ctx, B, n)}),
                          v);
          }
  ...
  if (!exit) {
    ...
    // case.
    errs() << "WARNING: ShadowMem: function `" << F.getName()
           << "' never returns\n";
    return true;
  }
  ...
  for (const DSNode *n : reach) {
    ...
    if ((n->isReadNode() || n->isModifiedNode()) && retReach.count(n) <= 0) {
      ...
      /// initial value
      B.CreateCall(m_markIn, {B.getInt32(getId(n)), inits[n], B.getInt32(idx),
                              getUniqueScalar(ctx, B, n)});
    }
