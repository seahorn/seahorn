...
HasErrorFunc("self-composition-has-error-function",
...
namespace seahorn
{
  ...
  BasicBlock* SelfComposition::createErrorBlock (Function &F,
                                            IRBuilder<> B, 
                                            AllocaInst* taintVar) {
    BasicBlock* errBB = 
        BasicBlock::Create(B.getContext (), 
                           "EqCheck_" + taintVar->getName().str(),
                           &F);
...
void SelfComposition::insertEqCheck(
        Function &F,
        IRBuilder<>& B,
        Instruction& inst,
        Value* var)
{
    ...
    Value *eqCheck = B.CreateICmpEQ (B.CreateLoad(var),
                                     B.CreateLoad(shadow),
                                     "eq_check");
...
Value* SelfComposition::allocaForValue (IRBuilder<>& B, const AllocaInst *v, uint64_t size)
{
    auto it = m_inst_to_shadow.find (v);
    if (it != m_inst_to_shadow.end ()) return it->second;
...
Value* SelfComposition::addShadowVar(IRBuilder<>& B, AllocaInst* var, uint64_t size)
{
...
GetElementPtrInst * SelfComposition::cloneOrigGEP(GetElementPtrInst *gep, Instruction *shadow)
{
    GetElementPtrInst *clone = dynamic_cast<GetElementPtrInst*>(gep->clone());
    unsigned idx_i=0;
    for (auto itIdx = clone->op_begin(), end = clone->op_end();
         itIdx != end; itIdx++, idx_i++)
    {
        Value *op = *itIdx;
        if (clone->getPointerOperand() == op) {
            if (isa<GetElementPtrInst>(op))
                *itIdx = cloneOrigGEP((GetElementPtrInst*)op, (Instruction*)m_inst_to_shadow[op]);
            continue;
        }
...
bool SelfComposition::isSCRequired(Instruction* inst)
{
         ...
         it++)
    {
        ...
        bool noTaint = false;//(taintIt != m_inst_to_taint.end()) && !taintIt->second;
...
bool SelfComposition::isUncomposable(Instruction *inst, Instruction *store)
{
    ...
    if (noTaint != m_inst_to_notaint.end() && m_dm.dominates((Instruction*)noTaint->second, store))
        return true;
...
void SelfComposition::basicBlockPass(IRBuilder<> &B, BasicBlock *bb)
{
    ...
    if (m_splitBB != nullptr &&
        m_postDm.dominates(bb, m_splitBB) &&
        bb->getTerminator()->getNumSuccessors() <= 1 &&
        (bb->getTerminator()->getNumSuccessors() == 0 ||
         !isa<PHINode>(bb->getTerminator()->getSuccessor(0)->getInstList().front())))
    {
        ValueToValueMapTy instMap;
        BasicBlock *shadowBB = CloneBasicBlock(bb, instMap, "_shadow", bb->getParent());
    ...
    if (m_splitBB != nullptr ||
        ((isSCRequired(terminator) || succHasPhi) && terminator->getNumSuccessors() > 1) ||
...
void SelfComposition::connectShadowBB(IRBuilder<> &B, BasicBlock *postDom)
{
    ...
    if (isa<BranchInst>(postDom->getTerminator())) {
        ...
        BranchInst *branchShadow = dynamic_cast<BranchInst*>(postDomShadow->getTerminator());
...
void SelfComposition::updateInstruction(IRBuilder<> &B, Instruction *inst)
{
    ...
    if (isa<StoreInst>(inst)) {
        ...
        if (shadowDst == m_inst_to_shadow.end()) {
            ...
            if (isa<GetElementPtrInst>(dst)) {
                Type *t = ((PointerType*)dynamic_cast<GetElementPtrInst*>(dst)->getPointerOperandType())->getElementType();
    ...
    if (isa<PHINode>(shadow)) {
        ...
        for (auto it = pred_begin(phi->getParent()), end = pred_end(phi->getParent());
             it != end; it++)
        {
            BasicBlock *p = *it;
        ...
        for (auto itB = phi->block_begin(), endB = phi->block_end();
             itB != endB;
             itB++, opIdx++, itVal++)
        {
            Value *inB = *itB;
        ...
        for (auto it = shadow->op_begin(), end = shadow->op_end();
             it != end;
             it++)
        {
            Value *op = *it;
    ...
    if (isa<StoreInst>(inst)) {
        ...
        if (isa<Instruction>(src)) {
            ...
            if ((isUncomposable((Instruction*)src, inst) ||
                 ...
                 //!isSCRequired(inst)) // XXX: This one is an optimization - if problem occurs, look here
            {
                Value *origDst = dst;
                Type *t = dst->getType();
                if (isa<GetElementPtrInst>(dst)) {
                    GetElementPtrInst *gep = cloneOrigGEP(
                            (GetElementPtrInst*)dst, (Instruction*)shadowDst);
                    origDst = gep;
  ...
  bool SelfComposition::runOnFunction (Function &F)
  {
    ...
    for (BasicBlock* bb : BBWorkList) {
        ...
        if (it != m_inst_to_shadow.end()) {
            ...
            for (auto instIt = bb->begin(), end = bb->end(), shadowIt = shadowBB->begin();
             ...
             instIt != end; instIt++)
        {
            ...
            if (it != m_inst_to_shadow.end()) {
                if (isa<BranchInst>(inst) &&
                    m_updatedBranches.find((BranchInst*)it->second) != m_updatedBranches.end()) {
                    continue;
                }
...
static llvm::RegisterPass<seahorn::SelfComposition>
X ("self-composition",
   "Do self composition");
