...
bool ShadowMemSeaDsa2::runOnModule(llvm::Module &M) {
  if (M.begin() == M.end())
    return false;

  dsa::GlobalAnalysis &dsa = getAnalysis<DsaAnalysis>().getDsaAnalysis();
...
void ShadowMemSeaDsa2::getAnalysisUsage(llvm::AnalysisUsage &AU) const {
  AU.setPreservesAll();
  AU.addRequiredTransitive<DsaAnalysis>();
  AU.addRequired<llvm::CallGraphWrapperPass>();
  AU.addRequired<llvm::UnifyFunctionExitNodes>();
}
namespace sea_dsa_impl {} // end namespace sea_dsa_impl
