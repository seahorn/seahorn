...
//   for (auto &gv : boost::make_iterator_range (sm.global_begin(),
//   sm.global_end ()))
...
/// outReach - subset of reach that is only reachable from the return node
template <typename Set1, typename Set2>
void argReachableNodes(const Function &fn, Graph &G, Set1 &reach,
                       Set2 &outReach) {
...
AllocaInst *ShadowMemSeaDsa::allocaForNode(const Node *n,
...
void ShadowMemSeaDsa::updateReadMod(Function &F, NodeSet &readSet,
                                    NodeSet &modSet) {
  if (!m_dsa->hasGraph(F))
    return;
      ...
      if (const AllocaInst *ai = dyn_cast<AllocaInst>(&inst)) {
        ...
        if (CS.getCallee()->getName().equals("calloc")) {
          ...
          if (c.getOffset() == 0) {
            ...
            errs() << "WARNING: missing calloc instrumentation because cell "
                      "offset is not zero\n";
          }
          ...
          if (c.getOffset() == 0) {
            ...
            // TODO: handle multiple nodes
            errs() << "WARNING: missing memset instrumentation because cell "
                      "offset is not zero\n";
          }
...
static llvm::RegisterPass<seahorn::ShadowMemSeaDsa> X("shadow-sea-dsa",
                                                      "Shadow DSA nodes");

static llvm::RegisterPass<seahorn::StripShadowMem>
    Y("strip-shadow-dsa", "Remove shadow.mem instrinsics");
