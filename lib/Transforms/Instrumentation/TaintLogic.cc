...
HasErrorFunc("taint-logic-has-error-function",
...
namespace seahorn
{
  ...
  BasicBlock* TaintLogic::createErrorBlock (Function &F,
                                            IRBuilder<> B, 
                                            AllocaInst* taintVar) {
    BasicBlock* errBB = 
        BasicBlock::Create(B.getContext (), 
                           "TaintCheck_" + taintVar->getName().str(), 
                           &F);
...
void TaintLogic::insertTaintCheck(
        Function &F,
        IRBuilder<>& B,
        Instruction& inst,
        Value* taintVar) {
    ...
    Value* taintCheck = B.CreateICmpEQ (B.CreateAlignedLoad(taintVar, 1),
                                        ConstantInt::get(m_BoolTy, 0),
                                        "taint_check");
...
Value* TaintLogic::allocaForValue (IRBuilder<>& B, const Value *v, uint64_t size)
{
...
bool TaintLogic::isTaintVarExists(const Value *val)
{
    if (isa<GetElementPtrInst>(val)) {
        const GetElementPtrInst& gep = dynamic_cast<const GetElementPtrInst&>(*val);
...
Value* TaintLogic::addShadowTaintVar(IRBuilder<>& B, Value* var, uint64_t size)
{
    ...
    if (size == 0)
        ...
        for (uint64_t i = 0; i < size; i++) {
            Value * indices[] = {0, ConstantInt::get(Type::getInt32Ty(B.getContext()), i)};
...
bool TaintLogic::isTaintLogicRequired(Instruction& inst)
{
    ...
    if (bbTaint != m_inst_to_shadow.end()) {
        if (isa<BranchInst>(inst) && dynamic_cast<BranchInst&>(inst).isUnconditional()) {
    ...
    if (isa<PHINode>(inst)) {
        ...
        for (unsigned i=0; i < size && !bTaint; i++) {
            if (m_inst_to_shadow.find(phi.getIncomingValue(i)) != m_inst_to_shadow.end())
                bTaint = true;
        }
    }
    ...
    if (isa<StoreInst>(inst)) {
        ...
        if (isa<GetElementPtrInst>(dst)) {
            ...
            if (it == m_inst_to_shadow.end()) {
                ...
                shadow = addShadowTaintVar(B, ptr, ((ArrayType*)t)->getNumElements());
            }
            else
                shadow = it->second;
            */
...
void TaintLogic::addAllShadowTaintVars(IRBuilder<>& B, std::vector<Instruction*>& WorkList)
{
    B.SetInsertPoint(&*m_topBB->getFirstInsertionPt());
...
void TaintLogic::insertTaintLogic(IRBuilder<>& B, Instruction& inst)
{
    ...
    Instruction* insertPoint = (isa<TerminatorInst>(inst)) ? &inst : inst.getNextNode();
    if (isa<PHINode>(inst))
    ...
    for (unsigned i=0; i < size; i++) {
        ...
        if (isa<StoreInst>(inst) && i == 1)
            ...
            dynamic_cast<GetElementPtrInst&>(inst).getPointerOperandIndex() == i) {
    ...
    for (unsigned e = 0; e < numElements; e++) {
        taint[e] = (size == 0) ? (Value*)ConstantInt::get(m_BoolTy, 0) : createLoadTaint(B, shadows[0], e);
        for (unsigned i=1; i < size; i++) {
            Value* tmp = B.CreateAlignedLoad(shadows[i], 1);
            taint[e] = B.CreateOr(taint[e], tmp, inst.getName() + "_" + std::to_string(e) + "_" + std::to_string(i));
        }
    }

    if (isa<SelectInst>(inst)) {
        assert(numElements == 1 && "No support for more than 1 for SelectInst - for now\n");
    ...
    if (!isa<BranchInst>(inst)) {
        ...
        {
            for (unsigned e=0; e < numElements; e++) {
                Value *g = B.CreateConstGEP2_32(ArrayType::get(m_BoolTy, numElements), a, 0, e);
...
static llvm::RegisterPass<seahorn::TaintLogic>
X ("taint-logic",
   "Insert taint tracking logic");
