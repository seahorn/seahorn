...
Expr OpSemMemHybridRepr::normalizePtr(Expr ptrE) {
  seahorn::ScopedStats _st("hybrid.ptr-norm");
  PointerArithmeticConfig paC(ptrE->efac(), m_cache);
  ExprRewriter<PointerArithmeticConfig> pRw(ptrE->efac(), paC, m_cache);
  return pRw.rewriteExpr(ptrE);
}
