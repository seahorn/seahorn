...
static llvm::cl::opt<unsigned>
    WordSize("horn-bv3-word-size", llvm::cl::desc("Word size in bytes: 1, 4"),
             cl::init(4));

static llvm::cl::opt<unsigned>
    PtrSize("horn-bv3-ptr-size", llvm::cl::desc("Pointer size in bytes: 4"),
            cl::init(4), cl::Hidden);

static llvm::cl::opt<bool> EnableUniqueScalars2(
    "horn-bv3-singleton-aliases",
    llvm::cl::desc("Treat singleton alias sets as scalar values"),
    cl::init(true));

static llvm::cl::opt<bool> InferMemSafety2(
    "horn-bv3-use-mem-safety",
...
static llvm::cl::list<std::string> IgnoreExternalFunctions2(
    "horn-bv3-ignore-external-functions",
    llvm::cl::desc(
...
const seahorn::details::Bv3OpSemContext &const_ctx3(const OpSemContext &_ctx);
} // namespace
...
class Bv3OpSemContext : public OpSemContext {
  friend class OpSem3Base;

private:
  /// \brief Set memory manager to be used by the machine
  void setMemManager(OpSem3Manager *man);
  ...
  using OpSem3ManagerPtr = std::unique_ptr<OpSem3Manager>;

  /// \brief Memory manager for the machine
  OpSem3ManagerPtr m_memManager;
  ...
  const Bv3OpSemContext *m_parent = nullptr;
...
public:
  ...
  Bv3OpSemContext(Bv3OpSem &sem, SymStore &values, ExprVector &side);
  /// \brief Clone a context with possibly new values and side condition
  /// \sa Bv3OpSem::fork
  Bv3OpSemContext(SymStore &values, ExprVector &side,
                  const Bv3OpSemContext &other);
  Bv3OpSemContext(const Bv3OpSemContext &) = delete;
  ~Bv3OpSemContext() override = default;
  ...
  OpSem3Manager *getMemManager() { return m_memManager.get(); }
  ...
  OpSemContextPtr fork(SymStore &values, ExprVector &side) {
    return OpSemContextPtr(new Bv3OpSemContext(values, side, *this));
  }

private:
  static Bv3OpSemContext &ctx3(OpSemContext &ctx) {
    return static_cast<Bv3OpSemContext &>(ctx);
  }
};
...
public:
  OpSem3Manager(Bv3OpSem &sem, Bv3OpSemContext &ctx)
  ...
  Expr storeAlignedWordToMem(Expr val, PtrTy ptr, Expr mem) {
    ...
    Expr addr =
        bind::mkConst(mkTerm<std::string>("addr", m_efac), bvTy);

    Expr cmp = mk<EQ>(addr, ptr);
    Expr fappl = op::bind::fapp(mem, ptr);
    Expr ite = boolop::lite(cmp, val, fappl);
  ...
  Expr MemFill(PtrTy dPtr, char *sPtr, unsigned len, uint32_t align = 0) {
    ...
    for (unsigned i = 0; i < len; i += sem_word_sz) {
      ...
      unsigned long word =
          0; // 8 bytes because assumed largest supported sem_word_sz = 8
...
struct OpSem3Base {
  ...
  OpSem3Base(Bv3OpSemContext &ctx, Bv3OpSem &sem)
    ...
    // XXX AG: this is probably wrong since instances of OpSem3Base are created
  ...
  Expr wrapWithLambda(Expr reg, Expr val) {
    if (!bind::isArrayConst(val))
      return val;

    Expr decl = bind::fname(val);
    Expr s = bind::rangeTy(decl);
    Expr idxTy = sort::arrayIndexTy(s);

    Expr bvT = bind::mkConst(mkTerm<std::string>("t", val->efac()), idxTy);
    Expr sel = op::array::select(val, bvT);
  ...
  Expr havoc(const Value &v) {
    ...
    Expr reg;
    if (reg = m_ctx.getRegister(v))
      return wrapWithLambda(reg, m_ctx.havoc(reg));

    assert(!isa<Constant>(v));
    reg = m_ctx.mkRegister(v);
    if (reg) {
      LOG("opsem3", errs() << "havoc reg: " << *reg << "\n");
      return wrapWithLambda(reg, m_ctx.havoc(reg));
    }
...
class OpSem3Visitor : public InstVisitor<OpSem3Visitor>, OpSem3Base {
public:
  OpSem3Visitor(Bv3OpSemContext &ctx, Bv3OpSem &sem) : OpSem3Base(ctx, sem) {}
  ...
  void visitAllocaInst(AllocaInst &I) {
    ...
    if (const Constant *cv = dyn_cast<const Constant>(I.getOperand(0))) {
      ...
      LOG("opsem", errs() << "!1 Alloca of " << memSz << " bytes: " << I
                          << ":\n\taddr " << addr << "\n");
    } else {
      Expr nElts = lookup(*I.getOperand(0));
      LOG("opsem", errs() << "!2 Alloca of " << nElts << "*" << typeSz
                          << " bytes: " << I << "\n";);
  ...
  Expr executeTruncInst(const Value &v, const Type &ty, Bv3OpSemContext &ctx) {
  ...
  Expr executeZExtInst(const Value &v, const Type &ty, Bv3OpSemContext &ctx) {
    if (v.getType()->isVectorTy()) {
      llvm_unreachable(nullptr);
    }
  ...
  Expr executeSExtInst(const Value &v, const Type &ty, Bv3OpSemContext &ctx) {
    if (v.getType()->isVectorTy()) {
      llvm_unreachable(nullptr);
    }
  ...
  Expr executeICMP_EQ(Expr op0, Expr op1, Type *ty, Bv3OpSemContext &ctx) {
    switch (ty->getTypeID()) {
    case Type::IntegerTyID:
  ...
  Expr executeICMP_NE(Expr op0, Expr op1, Type *ty, Bv3OpSemContext &ctx) {
    switch (ty->getTypeID()) {
    case Type::IntegerTyID:
  ...
  Expr executeICMP_ULT(Expr op0, Expr op1, Type *ty, Bv3OpSemContext &ctx) {
    switch (ty->getTypeID()) {
    case Type::IntegerTyID:
  ...
  Expr executeICMP_SLT(Expr op0, Expr op1, Type *ty, Bv3OpSemContext &ctx) {
    switch (ty->getTypeID()) {
    case Type::IntegerTyID:
  ...
  Expr executeICMP_UGT(Expr op0, Expr op1, Type *ty, Bv3OpSemContext &ctx) {
    switch (ty->getTypeID()) {
    case Type::IntegerTyID:
  ...
  Expr executeICMP_SGT(Expr op0, Expr op1, Type *ty, Bv3OpSemContext &ctx) {
  ...
  Expr executeICMP_ULE(Expr op0, Expr op1, Type *ty, Bv3OpSemContext &ctx) {
    switch (ty->getTypeID()) {
    case Type::IntegerTyID:
  ...
  Expr executeICMP_SLE(Expr op0, Expr op1, Type *ty, Bv3OpSemContext &ctx) {
    switch (ty->getTypeID()) {
    case Type::IntegerTyID:
  ...
  Expr executeICMP_UGE(Expr op0, Expr op1, Type *ty, Bv3OpSemContext &ctx) {
    switch (ty->getTypeID()) {
    case Type::IntegerTyID:
  ...
  Expr executeICMP_SGE(Expr op0, Expr op1, Type *ty, Bv3OpSemContext &ctx) {
    switch (ty->getTypeID()) {
    case Type::IntegerTyID:
  ...
  Expr executeGEPOperation(const Value &ptr, gep_type_iterator it,
                           gep_type_iterator end, Bv3OpSemContext &ctx) {
  ...
  Expr executeBitCastInst(const Value &op, Type *ty, Bv3OpSemContext &ctx) {
...
struct OpSemPhiVisitor : public InstVisitor<OpSemPhiVisitor>, OpSem3Base {
  OpSemPhiVisitor(Bv3OpSemContext &ctx, Bv3OpSem &sem) : OpSem3Base(ctx, sem) {}
...
namespace seahorn {
namespace details {
Bv3OpSemContext::Bv3OpSemContext(Bv3OpSem &sem, SymStore &values,
                                 ExprVector &side)
  ...
  setMemManager(new OpSem3Manager(m_sem, *this));
}

Bv3OpSemContext::Bv3OpSemContext(SymStore &values, ExprVector &side,
                                 const Bv3OpSemContext &o)
...
void Bv3OpSemContext::setMemManager(OpSem3Manager *man) {
...
Expr Bv3OpSemContext::loadValueFromMem(Expr ptr, const llvm::Type &ty,
                                       uint32_t align) {
  assert(m_memManager);
...
Expr Bv3OpSemContext::storeValueToMem(Expr val, Expr ptr, const llvm::Type &ty,
                                      uint32_t align) {
  assert(m_memManager);
  assert(getMemReadRegister());
...
Expr Bv3OpSemContext::MemSet(Expr ptr, Expr val, unsigned len, uint32_t align) {
  assert(m_memManager);
  assert(getMemReadRegister());
  assert(getMemWriteRegister());
...
Expr Bv3OpSemContext::MemCpy(Expr dPtr, Expr sPtr, unsigned len,
                             uint32_t align) {
  assert(m_memManager);
...
Expr Bv3OpSemContext::MemFill(Expr dPtr, char *sPtr, unsigned len,
                              uint32_t align) {
  assert(m_memManager);
  assert(getMemReadRegister());
  assert(getMemWriteRegister());
...
Expr Bv3OpSemContext::inttoptr(Expr intValue, const Type &intTy,
...
Expr Bv3OpSemContext::ptrtoint(Expr ptrValue, const Type &ptrTy,
...
Expr Bv3OpSemContext::gep(Expr ptr, gep_type_iterator it,
...
void Bv3OpSemContext::onFunctionEntry(const Function &fn) {
  m_memManager->onFunctionEntry(fn);
}
void Bv3OpSemContext::onModuleEntry(const Module &M) {
  LOG("opsem3", llvm::errs() << "onModuleEntry\n" << M << "\n");

  return m_memManager->onModuleEntry(M);
}

void Bv3OpSemContext::declareRegister(Expr v) { m_registers.insert(v); }
bool Bv3OpSemContext::isKnownRegister(Expr v) { return m_registers.count(v); }

Expr Bv3OpSemContext::mkRegister(const llvm::BasicBlock &bb) {
...
Expr Bv3OpSemContext::mkPtrRegisterSort(const Instruction &inst) const {
  if (m_parent)
    return m_parent->mkPtrRegisterSort(inst);
  return m_memManager->mkPtrRegisterSort(inst);
}

Expr Bv3OpSemContext::mkPtrRegisterSort(const Function &fn) const {
  if (m_parent)
    return m_parent->mkPtrRegisterSort(fn);
  return m_memManager->mkPtrRegisterSort(fn);
}

Expr Bv3OpSemContext::mkPtrRegisterSort(const GlobalVariable &gv) const {
  if (m_parent)
    return m_parent->mkPtrRegisterSort(gv);
  return m_memManager->mkPtrRegisterSort(gv);
}

Expr Bv3OpSemContext::mkMemRegisterSort(const Instruction &inst) const {
  if (m_parent)
    return m_parent->mkMemRegisterSort(inst);
  return m_memManager->mkMemRegisterSort(inst);
}

Expr Bv3OpSemContext::mkRegister(const llvm::Instruction &inst) {
  ...
  if (isShadowMem(inst, &scalar)) {
    ...
    if (scalar) {
      LOG("opsem3", llvm::errs() << "scalar cell: " << inst << "\n");
  ...
  LOG("opsem3", llvm::errs() << "New reg " << *reg << "\n");
...
Expr Bv3OpSemContext::mkRegister(const llvm::Function &fn) {
...
Expr Bv3OpSemContext::mkRegister(const llvm::GlobalVariable &gv) {
...
Expr Bv3OpSemContext::mkRegister(const llvm::Value &v) {
...
Expr Bv3OpSemContext::getConstantValue(const llvm::Constant &c) {
  // -- easy common cases
...
std::pair<char *, unsigned>
Bv3OpSemContext::getGlobalVariableInitValue(const llvm::GlobalVariable &gv) {
  return m_memManager->getGlobalVariableInitValue(gv);
}

Expr Bv3OpSemContext::boolToBv(Expr b) {
  if (isOpX<TRUE>(b))
    return trueBv;
  if (isOpX<FALSE>(b))
    return falseBv;
...
Expr Bv3OpSemContext::bvToBool(Expr bv) {
  if (bv == trueBv)
    return m_trueE;
  if (bv == falseBv)
    return m_falseE;
...
Bv3OpSem::Bv3OpSem(ExprFactory &efac, Pass &pass, const DataLayout &dl,
                   TrackLevel trackLvl)
...
OpSemContextPtr Bv3OpSem::mkContext(SymStore &values, ExprVector &side) {
  return OpSemContextPtr(new details::Bv3OpSemContext(*this, values, side));
}

Bv3OpSem::Bv3OpSem(const Bv3OpSem &o)
    : OperationalSemantics(o), m_pass(o.m_pass), m_trackLvl(o.m_trackLvl),
      m_td(o.m_td), m_canFail(o.m_canFail) {}

Expr Bv3OpSem::errorFlag(const BasicBlock &BB) {
...
void Bv3OpSem::exec(const BasicBlock &bb, details::Bv3OpSemContext &ctx) {
  ctx.onBasicBlockEntry(bb);

  details::OpSem3Visitor v(ctx, *this);
...
void Bv3OpSem::execPhi(const BasicBlock &bb, const BasicBlock &from,
...
Expr Bv3OpSem::symbolicIndexedOffset(gep_type_iterator TI, gep_type_iterator TE,
                                     details::Bv3OpSemContext &ctx) {
...
unsigned Bv3OpSem::pointerSizeInBits() const {
  return m_td->getPointerSizeInBits();
}

uint64_t Bv3OpSem::sizeInBits(const llvm::Type &t) const {
  return m_td->getTypeSizeInBits(const_cast<llvm::Type *>(&t));
}

uint64_t Bv3OpSem::sizeInBits(const llvm::Value &v) const {
  return sizeInBits(*v.getType());
}

unsigned Bv3OpSem::storageSize(const llvm::Type *t) const {
  return m_td->getTypeStoreSize(const_cast<Type *>(t));
}

unsigned Bv3OpSem::fieldOff(const StructType *t, unsigned field) const {
  return m_td->getStructLayout(const_cast<StructType *>(t))
      ->getElementOffset(field);
}

Expr Bv3OpSem::getOperandValue(const Value &v, details::Bv3OpSemContext &ctx) {
  Expr res;
...
bool Bv3OpSem::isSymReg(Expr v, details::Bv3OpSemContext &C) {
...
const Value &Bv3OpSem::conc(Expr v) const {
  assert(isOpX<FAPP>(v));
  // name of the app
  Expr u = bind::fname(v);
  // name of the fdecl
...
bool Bv3OpSem::isSkipped(const Value &v) const {
...
bool Bv3OpSem::intraStep(details::Bv3OpSemContext &C) {
  if (C.isAtBbEnd())
    return false;
  ...
  details::OpSem3Visitor v(C, *this);
  v.visit(const_cast<Instruction &>(inst));
  return res;
}

void Bv3OpSem::intraPhi(details::Bv3OpSemContext &C) {
  assert(C.getPrevBb());
...
void Bv3OpSem::intraBr(details::Bv3OpSemContext &C, const BasicBlock &dst) {
...
void Bv3OpSem::skipInst(const Instruction &inst,
                        details::Bv3OpSemContext &ctx) {
  const Value *s;
...
void Bv3OpSem::unhandledValue(const Value &v, details::Bv3OpSemContext &ctx) {
  if (const Instruction *inst = dyn_cast<const Instruction>(&v))
    return unhandledInst(*inst, ctx);
  LOG("opsem", WARN << "unhandled value: " << v;);
}
void Bv3OpSem::unhandledInst(const Instruction &inst,
                             details::Bv3OpSemContext &ctx) {
  if (ctx.isIgnored(inst))
    return;
  ctx.ignore(inst);
...
Expr Bv3OpSem::mkSymbReg(const Value &v, OpSemContext &_ctx) {
  return details::ctx3(_ctx).mkRegister(v);
}

Expr Bv3OpSem::getSymbReg(const Value &v, const OpSemContext &_ctx) const {
  return const_ctx3(_ctx).getRegister(v);
}

/// \brief Returns a concrete value to which a constant evaluates
/// Adapted from llvm::ExecutionEngine
Optional<GenericValue> Bv3OpSem::getConstantValue(const Constant *C) {
...
void Bv3OpSem::execBr(const BasicBlock &src, const BasicBlock &dst,
                      details::Bv3OpSemContext &ctx) {
...
seahorn::details::Bv3OpSemContext &ctx3(OpSemContext &_ctx) {
  return static_cast<seahorn::details::Bv3OpSemContext &>(_ctx);
}
} // namespace details
} // namespace seahorn
namespace {
...
const seahorn::details::Bv3OpSemContext &const_ctx3(const OpSemContext &_ctx) {
  return static_cast<const seahorn::details::Bv3OpSemContext &>(_ctx);
}
} // namespace
