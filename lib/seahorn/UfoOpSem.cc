...
struct OpSemVisitor : public InstVisitor<OpSemVisitor>, OpSemBase {
  ...
  void visitCallSite(CallSite CS) {
    ...
    if (F.getName().startswith("verifier.assume")) {
      ...
      assert(m_fparams.size() == 3);
      if (IgnoreCalloc)
        if (fmap::returnsFiniteMap(m_inMem))
      ...
      if (m_fparams.size() > 3) {
        ...
        if (m_sem.isAbstracted(*f)) {
          ...
          for (unsigned i = 0; i < m_inRegions.size(); i++) {
            if (fmap::returnsFiniteMap(m_inRegions[i]))
  ...
  void visitLoadInst(LoadInst &I) {
    ...
    if (m_uniq) {
      ...
      else if (fmap::returnsFiniteMap(m_inMem)) {
...
struct OpSemPhiVisitor : public InstVisitor<OpSemPhiVisitor>, OpSemBase {
  ...
  void visitBasicBlock(BasicBlock &BB) {
    ...
    for (unsigned i = 0; isa<PHINode>(curr); ++curr) {
      ...
      if (op0 && fmap::returnsFiniteMap(op0)) {
        write(phi, op0);
      } else {
        Expr lhs = havoc(phi);
        side(lhs, op0);
...
bool MemUfoOpSem::isMemScalar(const Cell &c) {
  ...
  return !(OpSemVisitor::returnsArray(memE) || fmap::returnsFiniteMap(memE));
}
...
void FMapUfoOpSem::execCallSite(CallSiteInfo &csi, ExprVector &side,
  ...
  auto addStore = [&](const Expr lhs, const Expr rhs, const Value *v) {
    if (OpSemVisitor::returnsArray(lhs))
      ...
      assert(fmap::returnsFiniteMap(rhs));
  ...
  for (int i = 3; i < csi.m_fparams.size(); i++) {
    ...
    } else if ((fmap::returnsFiniteMap(param) &&
    ...
    { // unused param
      // LOG("fmap-node", assert(!(fmap::returnsFiniteMap(param) &&
      //                           isOpX<ARRAY_TY>(fi.sumPred->arg(i + 1))) &&
      //                         "Oversharing"));
...
Expr FMapUfoOpSem::memObtainValue(Expr mem, Expr offset) {
  ...
  else if (fmap::returnsFiniteMap(mem))
...
void FMapUfoOpSem::storeVal(const Cell &cCr, const Cell &cSAS, Expr readFrom,
  ...
  getCellValues(cSAS, MemOpt::OUT)
      .push_back(fmap::mkVarGet(readFrom, dir, 0, sort::intTy(m_efac)));
}
