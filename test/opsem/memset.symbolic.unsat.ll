...
entry:
  ...
  ; Keep len symbolic, but constrain it to 6 so this still goes through the
  ; symbolic-length memset path while exercising a remainder update.
  %len = call i32 @nd()
  %is6 = icmp eq i32 %len, 6
  call void @verifier.assume(i1 %is6)

  ; Zero the whole buffer, then memset a symbolic number of bytes.
  ...
  ; The first word should always be fully overwritten.
  %word.ptr = bitcast i8* %ptr to i32*
  %word = load i32, i32* %word.ptr, align 4
  %word.ok = icmp eq i32 %word, 202116108

  ; The symbolic length is 6, so the remainder should update the next 2 bytes.
  ...
  ; Byte 6 is beyond the symbolic length, so it must stay untouched.
